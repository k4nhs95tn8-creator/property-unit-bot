...
def page(message=''):
    ...
    <section><h2>2. Download your GitHub database</h2><p>GitHub’s cloud runner downloads the official DLD Units dataset, builds the indexed database and publishes a compressed release every Sunday. Your bot checks GitHub automatically at startup and hourly.</p><pre>{html.escape(data_status)}</pre>{github_form}{refresh_form}<p><a href="/">Refresh download status</a></p><p>Your configured public GitHub repository runs the weekly build. Use a public repository and standard runner for free Actions. Your Telegram token stays on this computer.</p><p>A single size or bedroom match does not identify an exact unit. All compatible candidates are included in the cloud test report.</p></section>
